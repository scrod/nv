...
#
# Utility script to process XHTML files back into MMD
#
...
#
# If a list of files is received, the input from "test.html" will be output
...
if ($count == 0) {
	...
	MultiMarkdown::Support::ProcessXHTML2MMD($MMDPath, "", $data);
	...
	foreach(@ARGV) {
		...
		if ( -d $_ ) {
			...
			MultiMarkdown::Support::ProcessXHTML2MMD($MMDPath, $filename, $data);
		} else {
			system("perldoc $0");
		}
	}
...
XHTML2mmd - utility script for MultiMarkdown to convert XHTML back into
MultiMarkdown text.
...
This script is designed as a "reversal" for MultiMarkdown. It can convert a
series of text files from XHTML back into MultiMarkdown text files.
