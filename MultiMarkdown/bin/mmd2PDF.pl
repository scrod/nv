#!/usr/bin/env perl
#
# $Id: mmd2PDF.pl 534 2009-06-19 20:52:55Z fletcher $
#
# Utility script to process MultiMarkdown files into PDF, via LaTeX
#
...
if ($count == 0) {
	...
	foreach(@ARGV) {
		...
		if ( -d $_ ) {
			...
			MultiMarkdown::Support::ProcessMMD2PDF($MMDPath, $filename, $data);
		} else {
			system("perldoc $0");
		}
	}
...
series of text files into LaTeX, and then into PDF.
