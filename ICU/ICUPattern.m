...
-(void)setStringToSearch:(NSString *)aStringToSearchOver range:(NSRange)range {
	NSParameterAssert(aStringToSearchOver);	
	UChar *utf16String = calloc(range.length + 1, sizeof(UChar));
	[aStringToSearchOver getCharacters:utf16String range:range];
...
-(BOOL)matchesString:(NSString *)stringToMatchAgainst {
	ICUMatcher *m = [ICUMatcher matcherWithPattern:self overString:stringToMatchAgainst range:NSMakeRange(0, [stringToMatchAgainst length])];
	return [m matches];
}
#endif
